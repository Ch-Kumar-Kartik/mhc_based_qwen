...
from src.conversionV2 import load_mhc_model_v2, validate_equivalence_v2
...
def main():
    ...
    # Try to load as V2 first, fall back to V1
    try:
        mhc_model, _ = load_mhc_model_v2(
            args.mhc,
            device="cuda",
            torch_dtype=torch_dtype,
        )
        use_v2 = True
        logger.info("Loaded as V2 model")
    except Exception as e:
        logger.warning(f"Failed to load as V2, trying V1: {e}")
        ...
        use_v2 = False
        logger.info("Loaded as V1 model")
