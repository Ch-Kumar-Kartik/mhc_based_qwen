param(
    ...
    [string]$Tasks = "milu",
    [string]$OutputDir = "E:\mhc_based_qwen-2\output\benchmark_milu_vllm",
...
$baseOutput = Join-Path $OutputDir "milu_vllm_base_$timestamp.json"
$mhcOutput = Join-Path $OutputDir "milu_vllm_mhc_$timestamp.json"
...
Write-Host "Running MILU (vLLM) for base model: $BaseModel"
Invoke-MiluEval -ModelPath $BaseModel -OutputPath $baseOutput

Write-Host "Running MILU (vLLM) for mHC checkpoint: $MhcModel"
