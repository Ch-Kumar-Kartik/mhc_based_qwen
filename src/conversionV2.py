...
def convert_qwen3_to_mhc_v2(
    ...
    Args:
        ...
        trust_remote_code: Whether to allow execution/download of remote model code.
        local_files_only: Whether to only load from local HF cache / local paths.
...
def create_mhc_config_v2_from_qwen3(
    ...
    config = Qwen3MHCConfigV2(
        ...
        tie_word_embeddings=getattr(qwen3_config, 'tie_word_embeddings', False),
        # Some upstream configs omit rope_theta; fall back to the Qwen default when absent.
        rope_theta=getattr(qwen3_config, 'rope_theta', 1_000_000.0),
    ...
    # Copy rope_parameters if available (needed for RotaryEmbedding initialization)
    if hasattr(qwen3_config, 'rope_parameters') and qwen3_config.rope_parameters is not None:
        config.rope_parameters = qwen3_config.rope_parameters
    else:
        # Set default rope_parameters if not present
...
def load_mhc_model_v2(
    ...
    # Heuristic: paths with drive letters/backslashes/multi-segment slashes are usually local.
    ...
    if looks_like_local_path and not is_local:
        raise FileNotFoundError(
            f"Local checkpoint/model path not found: {model_path_obj}. "
            "If you intended a Hugging Face repo ID, use 'namespace/repo_name'."
        )
    
    model = Qwen3MHCForCausalLMV2.from_pretrained(
        str(model_path_obj) if is_local else model_path,
        torch_dtype=torch_dtype,
        device_map=device,
        local_files_only=(is_local or local_files_only),
        trust_remote_code=trust_remote_code,
    )
