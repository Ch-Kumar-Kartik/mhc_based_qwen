...
set STEP1_OUT=output\qwen3_mhc_v2_parity_hc_only_3k
set STEP1_CKPT=%STEP1_OUT%\checkpoint-3000
set STEP2_OUT=output\qwen3_mhc_v2_parity_mixed_3k

:model_found
echo [INFO] Step 1: HC-only training (3k steps) starting...
E:\mhc_based_qwen-2\qwvenv2\Scripts\python.exe -u train.py --tokenized-dir data/sangraha_packed/ --model output/qwen3_mhc_v2_converted_20260330_parity --base-model %BASE_MODEL% --output-dir %STEP1_OUT% --batch-size 1 --gradient-accumulation-steps 8 --total-steps 3000 --freeze-original-steps 3000 --learning-rate 1e-4 --warmup-steps 300 --save-steps 500 --eval-steps 500 --keep-last-checkpoints 3 --num-workers 0 --no-persistent-workers --prefetch-factor 1 --pin-memory --dtype auto --local-files-only --no-resume --log-interval 20 --train-only-hyper-connection-params 1>>E:\mhc_based_qwen-2\train.out_parity_hc_only_3k.log 2>>E:\mhc_based_qwen-2\train.err_parity_hc_only_3k.log
if errorlevel 1 (
	echo [ERROR] Step 1 HC-only training failed. Check train.err_parity_hc_only_3k.log
	exit /b 1
)
